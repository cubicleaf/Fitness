...
(async()=>{
 ...
 try {
  ...
  const recommend=async()=>modal.getByRole('button',{name:'Recommend split',exact:true}).click();
  assert.equal(await modal.getByRole('button',{name:'Recommend split',exact:true}).isDisabled(),true);
  await input.fill('bench press');await recommend();await page.getByTestId('accept-split').waitFor();
  ...
  await recommend();await page.getByRole('button',{name:'Use No Split',exact:true}).waitFor();
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  await open();await page.getByPlaceholder('E.g., Bench Press').fill('plank plus press');
  await recommend();await page.waitForTimeout(450);assert.match(await helper.innerText(),/do not recognize/i);
  assert.equal(await page.getByTestId('accept-split').count(),0);
  ...
  await recommend();await page.getByTestId('accept-split').waitFor();
